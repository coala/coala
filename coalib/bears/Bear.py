...
class Bear(Printer, LogPrinterMixin, metaclass=bearclass):
    ...
    def debugger_config(self,*args,**kwargs):
        if ('linter' in str(self)) and (isinstance(self.run(*args, **kwargs), types.GeneratorType)):
            ...
            debug_mode_function(self.run,*args,**kwargs)
        elif (isinstance(self.run(*args, **kwargs),(types.NoneType,types.StringType))) and (u'wrapping_function of' in str(self.run)) or (u'_new_func of' in str(self.run)):
    ...
    def setup_debuger(self,*args,**kwargs):
        ...
        if debug_bears is False:
            ...
            # if (u'wrapping_function of' in str(self.run)) or (u'_new_func of' in str(self.run)):
        return None,args,kwargs
