...
class TestInit(unittest.TestCase):
    ...
    def test_raises(self):
        self.assertRaises(TypeError, FilterCollector, "kind", self.lp, [], "string", [], [])
        self.assertRaises(TypeError, FilterCollector, "kind", self.lp, [], [], "string", [])
        self.assertRaises(TypeError, FilterCollector, "kind", self.lp, [], [], [], "string")

        self.assertEqual(FilterCollector("kind", self.lp, [])._regexs, [])
...
class TestFilter(Analyser):
    def __init__(self):
        Analyser.__init__(self, "settings", "msg_queue")
...
""".format(first_file_name)
    ...
    def test_ignored(self):
        uut = FilterCollector("kind",
                              self.lp,
                              filter_dirs=[self.tmp_dir],
                              ignored=[os.path.splitext(os.path.basename(self.testfile1_path))[0]])
        filter_list = uut.collect()
        self.assertEqual(len(filter_list), 1)
