...
def get_context(console_printer, file_dict,
    ...
    :yield:                 Affected file's contextual information

    """

    related_lines = list()
    related_lines_nr = list()

    affected_line = file_dict[sourcerange.file][
                    sourcerange.start.line-1].rstrip()
    affected_line = affected_line.replace('\t', 8*' ')

    indet_space_count = len(affected_line)-len(affected_line.lstrip())
    if indet_space_count > 0:
        for j in range(sourcerange.start.line-2, -1, -1):  # pragma: no cover
            rline = file_dict[sourcerange.file][j].rstrip()
            affected_line = rline.replace('\t', 8*' ')
            prv_space_count = len(affected_line) - len(affected_line.lstrip())
            if prv_space_count < indet_space_count and rline:
                related_lines.append(rline)
                related_lines_nr.append(j+1)
                indet_space_count = prv_space_count
            if indet_space_count == 0:
                break
    else:
        ...
        for i in pre_context:
            ...
            related_lines_nr.append(i+1)

    related_lines_nr.reverse()
    related_lines.reverse()

    return related_lines, related_lines_nr
