...
def collect_files(file_paths):
    ...
    """
    results = list(icollect(file_paths))
...
def collect_dirs(dir_paths):
    ...
    """
    results = list(icollect(dir_paths))
    return [result for result in results if os.path.isdir(result)]
...
def icollect_bears(bear_dirs, bear_names, kinds, log_printer):
    ...
    """
    for bear_dir in icollect(bear_dirs):
        if os.path.isdir(bear_dir):
