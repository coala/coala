...
def icollect(file_paths, ignored_globs=None, match_cache={}):
    ...
    :param ignored_globs:
        List of globs to ignore when matching files
    :param match_cache:
        Dictionary to use for caching results
    :return:
...
def collect_dirs(dir_paths, ignored_dir_paths=None):
    ...
    :param dir_paths:
        File path or list of such that can include globs
    :param ignored_dir_paths:
        List of globs that match to-be-ignored dirs
    :return:
...
def icollect_bears(bear_dir_glob, bear_globs, kinds, log_printer=None):
    ...
    :param bear_dir_glob:
        Directory globs or list of such that can contain bears
    :param bear_globs:
        Globs of bears to collect
    :param kinds:
        List of bear kinds to be collected
    :param log_printer:
        Log_printer to handle logging
    :return:
        Iterator that yields a tuple with bear class and
...
def collect_bears(bear_dirs, bear_globs, kinds, log_printer=None,
    ...
    :param bear_dirs:
        Directory name or list of such that can contain
        bears.
    :param bear_globs:
        Globs of bears to collect.
    :param kinds:
    ...
    :param log_printer:
        log_printer to handle logging.
    :param warn_if_unused_glob:
        True if warning message should be shown if a
        glob didn't give any bears.
    :return:
        Tuple of list of matching bear classes based on
...
def filter_section_bears_by_languages(bears, languages):
    ...
    :param bears:
        The dictionary of the sections as keys and list of
        bears as values.
    :param languages:
        Languages that bears are being filtered on.
    :return:
        New dictionary with filtered out bears that don't match
...
def collect_bears_by_aspects(aspects, kinds):
    ...
    :param aspects:
        An AspectList that need to be covered.
    :param kinds:
        List of bear kinds to be collected.
    :return:
        Tuple of list of bear classes based on kind. The lists are
...
def filter_capabilities_by_languages(bears, languages):
    ...
    :param bears:
        Dictionary with sections as keys and list of bears as
        values.
    :param languages:
        Languages that bears are being filtered on.
    :return:
        New dictionary with languages as keys and their bears
...
def collect_all_bears_from_sections(sections, log_printer=None):
    ...
    :param sections:
        List of sections so bear_dirs are taken into account
    :param log_printer:
        Log_printer to handle logging
    :return:
        Tuple of dictionaries of local and global bears.
...
def _warn_if_unused_glob(globs, used_globs, message):
    ...
    :param message:
        Warning message to display if a glob is unused.
...
def collect_registered_bears_dirs(entrypoint):
    ...
    :param entrypoint:
        The entrypoint to find packages with.
    :return:
        List of bear directories.
    """
    collected_dirs = []
