    ...
    def fill_settings(self, filters):
        ...
        for filter in filters:
            if not hasattr(filter, "get_needed_settings"):
                ...
                needed_settings.update(filter.get_needed_settings())

        # Strip away existent settings.
        for settings_key in needed_settings.keys():
            if settings_key in self.settings:
                del needed_settings[settings_key]

        # Get missing ones.
        if len(needed_settings) > 0:
            new_vals = self.outputter.require_settings(needed_settings)
            for key, val in new_vals.items():
                self.settings.append(Setting(key, val))
