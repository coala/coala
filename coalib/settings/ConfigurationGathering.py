...
def save_toml_sections(sections):
    """
    Saves the given sections in the specified TOML configuration
    file if --save is used.
    ...
    try:
        if bool(default_section.get('save', 'false')):
            conf_writer = TomlConfWriter(
                str(default_section.get('config',
                                        Constants.local_coafile_toml)))
        else:
            return
    except ValueError:
        conf_writer = TomlConfWriter(str(default_section.get('save',
                                                             '.coafile.toml')))

    conf_writer.write(sections)
...
def gather_configuration(acquire_settings,
    ...
    if bool(sections['cli'].get('toml_config', 'False')):
        save_toml_sections(sections)
    else:
