...
class coalaDebugFlagTest(unittest.TestCase):
    ...
    def test_no_ipdb(self, mocked_mode_json):
        ...
        with bear_test_module(), prepare_file(['#fixme  '], None) as (lines, filename), self.pipReqIsNotInstalledMock():
    ...
    def test_bear__init__raises(self, mocked_mode_json):
        ...
        with bear_test_module(), prepare_file(['#fixme  '], None) as (lines, filename), self.pipReqIsInstalledMock(), patch.dict('sys.modules', ipdb=mocked_ipdb), self.assertRaisesRegex(
                    RuntimeError,
    ...
    def test_coala_main_mode_json_launches_ipdb(self, mocked_mode_json):
        ...
        mocked_ipdb = self.ipdbMock()
        with bear_test_module(), prepare_file(['#fixme  '], None) as (lines, filename), self.pipReqIsInstalledMock(), patch.dict('sys.modules', ipdb=mocked_ipdb), self.assertRaisesRegex(RuntimeError,
