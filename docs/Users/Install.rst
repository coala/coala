...
Now your prompt should look something similar to this
...
Using virtualenvironment helps keeping your global python interpreter clean.
...
::
    ...
    $ git clone https://github.com/coala-analyzer/coala-bears.git
    $ cd coala-bears
...
::
    ...
    $ python3 setup.py install --prefix=/your/prefix/location

Other options are documented on
https://docs.python.org/3.3/install/#alternate-installation.

.. note::

    If you are behind a proxy server, follow these steps:
    ...
    ::
        ...
        $ sudo -E pip install coala

    the ``-E`` flag takes the existing environment variables into the ``sudo``
    environment. i.e the values stored in ``http_proxy`` and ``https_proxy``

    You could also set your ``pip.conf`` file to use a proxy,
